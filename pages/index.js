...
export const getServerSideProps = async () => {
  const [projects, contacts, my, services] = await Promise.all([
    getAllContentByModelTypeId(TTypeId.Project),
    getAllContentByModelTypeId(TTypeId.Contact),
    getAllContentByModelTypeId(TTypeId.My),
    getAllContentByModelTypeId(TTypeId.Service),
  ]);
...
export default function Home({
  ...
  const { name, subTitle, location, summery } = my.fields;
  // Ref
      ...
      <div className="container mx-auto mb-10">
        ...
        <div className="laptop:mt-20 mt-10">
          <div className="mt-5">
            ...
            <h1
              ref={textThree}
              className="text-2xl tablet:text-3xl laptop:text-3xl laptopl:text-6xl p-1 tablet:p-2 text-bold w-full laptop:w-4/5"
            >
              Composing Wealth with Web3 Innovations
            </h1>
            <h1
              ref={textFour}
              className="text-xl tablet:text-2xl laptop:text-2xl laptopl:text-3xl p-1 tablet:p-2 text-bold w-full laptop:w-4/5"
            >
              working remotely - based in {location}
            </h1>
          </div>

          <Socials className="mt-2 laptop:mt-5" contacts={contacts} />
        </div>
